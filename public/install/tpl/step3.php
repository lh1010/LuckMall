      ...
      <div class="server">
        <table width="100%">
          ...
          <tr>
            ...
            <td><input type="text" name="dbname" id="dbname" value="luckmall" class="input" onBlur="TestDbPwd(0)"></td>
            <td><div id="J_install_tip_dbname"></div></td>
          </tr>
          <tr>
            <td class="tar">数据库表前缀：</td>
            <td><input type="text" name="dbprefix" id="dbprefix" value="luck_" class="input" ></td>
...
{
  // 格式化提示信息
  $('#J_install_tip_dbpw').html(""); $('#J_install_tip_dbname').html('');
  ...
  $.ajax({
      ...
      success: function(msg) {
          if (msg == 1) {
            if (connect_db == 1) $("#J_install_form").submit();
          } else if (msg == -1) {
              $('#J_install_tip_dbpw').html('<span for="dbname" generated="true" class="tips_error">请将sql-mode或sql_mode修改为NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION</span>');
          } else if(msg == -2) {
              $('#J_install_tip_dbname').html('<span for="dbname" generated="true" class="tips_error">不是空数据库, 请更换数据库名字</span>');
          } else {
              $('#dbpw').val("");
